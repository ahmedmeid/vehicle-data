...
@RestController
@RequestMapping("/api/vehicle-data-kafka")
public class VehicleDataKafkaResource {

    private final Logger log = LoggerFactory.getLogger(VehicleDataKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public VehicleDataKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
